...
//     let memory_used = final_memory - initial_memory; // Convert bytes to KB
...
fn main() {
    ...
    // Create a System instance to fetch memory statistics
    let system = System::new();
    let memory_info = system.memory().unwrap_or_else(|err| {
        eprintln!("Failed to get memory info: {}", err);
        std::process::exit(1); // Exit if we can't get memory info
    });

    // Measure memory usage before processing
    let initial_memory = memory_info.total.as_u64() - memory_info.free.as_u64(); // Calculate used memory

    // Call process_data with the input data
    let (product, total_sum) = fibonacci_performance::process_data(&data_input);

    // Adding a small delay for measurement
    ...
    // Measure memory usage after processing
    let final_memory = system.memory().unwrap().total.as_u64() - system.memory().unwrap().free.as_u64(); // Calculate used memory

    // Calculate memory used during processing
    let memory_used = final_memory - initial_memory; // Keep memory usage in bytes
